...
top_idx <- order(abs(coef_ridge), decreasing = TRUE)[1:1000]
...
# --- 7. Train final Lasso ---
lasso_final <- glmnet(X_train, y_train, family = "binomial", alpha = 0, lambda = lambda_best)
```

```{r}
# Prédictions sur le test
yhat_test = predict(lasso_final, newx=X_test, type="response")
yhat_class <- ifelse(yhat_test > 0.03, "Dead", "Alive")
table(y_test, yhat_class)
# Taux de précision
mean(yhat_class == y_test)
```
