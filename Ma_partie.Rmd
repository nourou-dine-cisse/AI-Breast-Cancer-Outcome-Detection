...
```{r}
# 1. Choose the fixed seed and put it BEFORE the sampling.
set.seed(42) 
...
n <- nrow(Cdata)
Cdata_scaled <- Cdata
# num_cols must contain only the names of numeric columns (you defined this earlier)
num_cols_names <- colnames(Cdata)[sapply(Cdata, is.numeric)]
Cdata_scaled[, num_cols_names] <- scale(Cdata_scaled[, num_cols_names])

# The split depends on set.seed(42)
train_idx <- sample(1:n, size = 0.7 * n)

train <- Cdata_scaled[train_idx, ]
test <- Cdata_scaled[-train_idx, ] # Use the setdiff method from your original code if preferred

# Target
y_train <- train$vital_status
y_test <- test$vital_status

# Features (This is the critical step for high dimensionality)
# NOTE: If you need to remove zero-variance features, that code should be run here 
# before calling cv.glmnet.
...
# --- Model Fitting (With increased control) ---

# 2. Lasso with CV (MUST include the control argument!)
cv <- cv.glmnet(
  X_train, 
  y_train, 
  family = "binomial", 
  alpha = 1,
  control = glmnet.control(maxit = 200000) 
)
...
# 1. Use your final, chosen seed
set.seed(42) 

# ... (Data splitting and X_train/y_train preparation) ...
...
cv <- cv.glmnet(
  ...
  family = "binomial", 
  alpha = 1,
  # This is the line that was likely missing or set to the default value
  control = glmnet.control(maxit = 200000) 
  # Include weights = sample_weights if you were using them
)

# best lambda
lambda_best <- cv$lambda.min

# 3. Train final lasso model (Good practice to include the control here too)
lasso <- glmnet(
  X_train, 
  y_train, 
  family = "binomial", 
